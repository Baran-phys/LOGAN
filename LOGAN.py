...
class Generator(nn.Module):
  def __init__(self, G_ch=64, G_depth=2, dim_z=256, bottom_width=4, resolution=256,
               G_kernel_size=3, G_attn='64', n_classes=40,
               num_G_SVs=1, num_G_SV_itrs=1,
               G_shared=True, shared_dim=128, hier=False,
               cross_replica=False, mybn=False,
               G_activation=nn.ReLU(inplace=False),
               G_lr=5e-5, G_B1=0.0, G_B2=0.999, adam_eps=1e-8,
               BN_eps=1e-5, SN_eps=1e-12, G_init='ortho', G_mixed_precision=False, G_fp16=False, 
  ...
  def forward(self, z, y):
    ...
    h = self.linear(z)
    # Reshape
    h = h.view(h.size(0), -1, self.bottom_width, self.bottom_width) #1st_layer:(batch,16384)-->(batch,1024,,4,4) for ch=64   
    # Loop over blocks
